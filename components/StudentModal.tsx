...
interface StudentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (student: Omit<Student, 'id' | 'attendanceId' | 'avgScore' | 'attendanceRate' | 'homeworkRate'> & { id?: number }) => void;
...
const StudentModal: React.FC<StudentModalProps> = ({ isOpen, onClose, onSave, student, allStudents, classes, teachers }) => {
  const [formData, setFormData] = useState<Omit<Student, 'id' | 'attendanceId' | 'avgScore' | 'attendanceRate' | 'homeworkRate'>>({
      name: '',
      gender: '남',
      school: '',
      grade: '',
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-h-[70vh] overflow-y-auto p-2">
              ...
              <div className="space-y-4">
                ...
                <div>
                 ...
                 <div>
                  <label htmlFor="parent1Phone" className="block text-sm font-medium text-gray-300 mb-1">학부모 연락처 1</label>
                  <input type="tel" name="parent1Phone" id="parent1Phone" value={formData.parent1Phone} onChange={handleChange} required className="w-full bg-gray-800 border border-gray-600 rounded-md p-2 text-white focus:ring-[#E5A823] focus:border-[#E5A823]" />
                </div>
              </div>
              ...
              {/* Column 3 */}
               <div className="space-y-4">
                 <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">형제 관계</label>
                  <p className="text-xs text-gray-400">(미구현)</p>
                  {/* <select multiple name="siblings" value={formData.siblings.map(String)} onChange={handleSiblingsChange} className="w-full h-24 bg-gray-800 border border-gray-600 rounded-md p-2 text-white">
                      {allStudents.filter(s => s.id !== student?.id).map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                  </select> */}
                 </div>
                 <div>
                 ...
                 <div>
                  <label htmlFor="parent2Phone" className="block text-sm font-medium text-gray-300 mb-1">학부모 연락처 2</label>
                  <input type="tel" name="parent2Phone" id="parent2Phone" value={formData.parent2Phone} onChange={handleChange} className="w-full bg-gray-800 border border-gray-600 rounded-md p-2 text-white focus:ring-[#E5A823] focus:border-[#E5A823]" />
                </div>
